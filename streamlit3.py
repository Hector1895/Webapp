...
#input features
early_return_amount =st.sidebar.number_input("Early return amount:",min_value=0, max_value=20000)
early_return_times =st.sidebar.slider("Early return times:",min_value=0, max_value=5,step=1)
early_return_amount_3mon =st.sidebar.number_input("Early return amount 3 month:",min_value=0.0, max_value=20000.0)
credit_class =st.sidebar.selectbox('Please choose credit class', ("A","B","C","D","E","F","G"))
interest =st.sidebar.number_input("Interest rate(%):",min_value=0.0, max_value=100.0)
debt_loan_ratio =st.sidebar.number_input("Debt loan ratio(%):",min_value=0.0, max_value=999.9)
year_of_loan =st.sidebar.slider("Year of Loan:",min_value=0, max_value=30,step=1)


def preprocess(early_return_amount, early_return_times,early_return_amount_3mon,credit_class,interest,debt_loan_ratio,year_of_loan):
    # Pre-processing user input
    user_input_dict={'early_return_amount':[early_return_amount], 'early_return':[early_return_times],
                     'early_return_amount_3mon':[early_return_amount_3mon],'class':[credit_class],
                     'interest':[interest],'debt_loan_ratio':[debt_loan_ratio],'year_of_loan':[year_of_loan]}
    ...
    cleaner_type = {"class": {'A':1,'B':2,'C':3,'D':4,'E':5,'F':6,'G':7}}
...
subtab_EDA, subtab_PREDICT = st.tabs(['Exploratory Data Analysis', 'Prediction'])
with subtab_EDA:
    # EDA
    st.subheader("1.Exploratory Data Analysis")

    train_df = pd.read_csv('mycsvfile.csv')

    cat_features = list(train_df.select_dtypes(include=['object']).columns)
    con_features = list(train_df.select_dtypes(include=['float64','int64']).columns)

    column_show = st.selectbox('Choose Feature', con_features, help = 'Filter report to show only one feature')

    # 布局尝试1
    # fig,axes =plt.subplots(nrows=4,ncols=1,figsize = (30,30), dpi = 150)
    # sns.histplot(x=train_df[column_show],ax=axes[0],bins=20,kde=False)
    # sns.boxplot(x = train_df['isDefault'],y = train_df[column_show],ax=axes[1])
    # sns.violinplot(x = train_df['isDefault'],y = train_df[column_show],ax=axes[2])
    # sns.kdeplot(train_df.loc[train_df['isDefault']==0,column_show],label='isDefault=0',ax=axes[3])
    # sns.kdeplot(train_df.loc[train_df['isDefault']==1,column_show],label='isDefault=1',ax=axes[3])
    # plt.legend()
    #
    # st.pyplot(fig)
    ...
    sns.histplot(x=train_df[column_show],bins=20,kde=False)
    st.pyplot(fig1)

    col1, col2 = st.columns([1, 1])
    with col1:
        fig2 = plt.figure()
        sns.boxplot(x=train_df['isDefault'], y=train_df[column_show])
        st.pyplot(fig2)
    with col2:
        fig3 = plt.figure()
        sns.violinplot(x = train_df['isDefault'],y = train_df[column_show])
        st.pyplot(fig3)


    fig4 = plt.figure(figsize=(10,5))
    sns.kdeplot(train_df.loc[train_df['isDefault']==0,column_show],label='isDefault=0')
    sns.kdeplot(train_df.loc[train_df['isDefault']==1,column_show],label='isDefault=1')
    plt.legend()
    st.pyplot(fig4)
...
with subtab_PREDICT:
    st.subheader("2.To predict default/ failure to pay back status, you need to follow the steps below:")
    st.markdown("""
    ...
    user_input = preprocess(early_return_amount, early_return_times, early_return_amount_3mon, credit_class, interest,debt_loan_ratio, year_of_loan)
    ...
    if btn_predict:
        ...
        if pred[0] > 0.3:
            ...
            st.write(f'Predicted default: {round(pred[0], 2)}')
        else:
            st.success('It is green! The aplicant has a high probability to pay the loan back!')
            st.write(f'Predicted default: {round(pred[0], 2)}')

        # prepare test set for shap explainability
        loans = st.cache_data(pd.read_csv)("mycsvfile.csv")
        cleaner_type = {"class": {'A': 1, 'B': 2, 'C': 3, 'D': 4, 'E': 5, 'F': 6, 'G': 7}}
        loans = loans.replace(cleaner_type)

        X = loans[['early_return_amount', 'early_return', 'early_return_amount_3mon', 'class', 'interest', 'debt_loan_ratio','year_of_loan']]
        y = loans[['isDefault']]
